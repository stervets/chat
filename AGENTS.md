...
- локальный Playwright traffic-check не должен создавать временные `traffic-room-*` комнаты: проверка трафика гоняется на существующих `Общий чат` / `King` и direct с `marx`, чтобы не засорять навигацию тестовым мусором.
- техническая чистка без изменения протоколов и пользовательского поведения: удалены неиспользуемые frontend helpers/типы/константы (`utils.ts`, `ws-handlers.ts`, `APP_TITLE/API_PREFIX/WS_PATH`, `COLOR_HEX_FULL_RE`, `MAX_ACTIVE_BROWSER_NOTIFICATIONS`, legacy `Invite/Session/WsEnvelope`) и backend legacy wrappers/types (`common/types.ts`, unused node/room/king/max-reserve exports), а также локальные backup/dist-артефакты;
- frontend WS-клиент упрощён: сборка MARX packet вынесена в один путь, закрытие сокета и sender/recipient вычисляются без дублей, reserve request dedupe использует общий `Set`, reserve runtime config/prompt/connect fallback очищены от повторяющихся веток;
- backend `chat.commands.ts` упрощён общими helpers для online-user enrichment, снятия `ok`-флага, room-aware broadcast и game event/message broadcast; поведение команд и payload-форматы сохранены;
- в chat navigation/profile модуле frontend повторяющиеся clone-ветки для `roomSurface`, dialogs и direct dialogs заменены короткими typed helpers, чтобы кеши/стейт продолжали получать независимые копии без копипасты.
- `/chat` frontend дополнительно сжат: однотипные on/off event subscriptions сведены к двум binding tables, а четыре метода проигрывания звуков сведены к одному `playSoundEffect` wrapper с сохранением прежних public method names.
